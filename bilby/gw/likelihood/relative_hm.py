...
class RelativeBinningHMGravitationalWaveTransient(GravitationalWaveTransient):
    """A gravitational-wave transient likelihood object which uses the relative
    binning procedure for higher modes to calculate a fast likelihood.
    See Leslie at al. :
    ...
    def __init__(
        ...
        if update_fiducial_parameters:
            ...
            logger.info(
                "Using scipy optimization to find maximum likelihood parameters."
            )
            self.parameters_to_be_updated = [
                key
                for key in self.priors
                if not isinstance(self.priors[key], (DeltaFunction, Constraint))
            ]
            logger.info(
                "Parameters over which likelihood is maximized: {}".format(
                    self.parameters_to_be_updated
                )
            )
            if parameter_bounds is None:
                ...
                self.parameter_bounds = self.get_parameter_list_from_dictionary(
                    parameter_bounds
                )
    ...
    def __repr__(self):
        return (
            ...
            + "(interferometers={},\n\twaveform_generator={},\n\fiducial_parameters={},".format(
                self.interferometers, self.waveform_generator, self.fiducial_parameters
            )
        )
    ...
    def setup_bins(self):
        ...
        for interferometer in self.interferometers:
            ...
            d_alpha = (
                ...
                * np.pi
                / np.abs(
                    (interferometer.minimum_frequency**gamma)
                    * np.heaviside(-gamma, 1)
                    - (interferometer.maximum_frequency**gamma)
                    * np.heaviside(gamma, 1)
                )
            )
            d_phi = np.sum(
                np.array(
                    [
                        np.sign(gamma[i])
                        * d_alpha[i]
                        * (frequency_array_useful ** gamma[i])
            ...
            self.bin_inds[interferometer.name] = np.zeros(
                self.number_of_bins + 1, dtype=np.int
            )

            for i in range(self.number_of_bins + 1):
                bin_index = np.where(
                    d_phi_from_start
                    >= ((i / self.number_of_bins) * d_phi_from_start[-1])
                )[0][0]
                bin_freq = frequency_array_useful[bin_index]
                self.bin_freqs[interferometer.name][i] = bin_freq
                self.bin_inds[interferometer.name][i] = np.where(
                    frequency_array >= bin_freq
                )[0][0]

            logger.info(
                "Set up {} bins for {} between {} Hz and {} Hz".format(
                    self.number_of_bins,
                    interferometer.name,
            ...
            self.waveform_generator.waveform_arguments[
                "frequency_bin_edges"
            ] = self.bin_freqs[interferometer.name]
        return
    ...
    def set_fiducial_waveforms(self, parameters, mode_array):
        ...
        for interferometer in self.interferometers:
            logger.info(
                "Maximum Frequency is {}".format(interferometer.maximum_frequency)
            )
            self.per_detector_per_mode_fiducial_waveforms[interferometer.name] = dict()
            for l_mode, m_mode in self.mode_array:
                parameters["mode_array"] = [[l_mode, m_mode]]
                self.fiducial_polarizations = (
                    self.waveform_generator.frequency_domain_strain(parameters)
                )

                maximum_nonzero_index = np.where(
                    self.fiducial_polarizations["plus"] != 0j
                )[0][-1]
                logger.info("Maximum Nonzero Index is {}".format(maximum_nonzero_index))
                maximum_nonzero_frequency = self.waveform_generator.frequency_array[
                    maximum_nonzero_index
                ]
                logger.info(
                    "Maximum Nonzero Frequency is {}".format(maximum_nonzero_frequency)
                )
                ...
                self.per_detector_per_mode_fiducial_waveforms[interferometer.name][
                    l_mode, m_mode
    ...
    def log_likelihood_ratio(self):
        ...
        complex_matched_filter_snr = 0.0
        ...
        if self.time_marginalization:
            log_l = self.time_marginalized_likelihood(
                d_inner_h_tc_array=d_inner_h_tc_array, h_inner_h=optimal_snr_squared
            )
            if self.jitter_time:
                self.parameters["geocent_time"] -= self.parameters["time_jitter"]
    ...
    def find_maximum_likelihood_parameters(
        self, parameter_bounds, iterations=1, maximization_kwargs=dict()
    ):
        ...
        for i in range(iterations):
            ...
            updated_parameters = self.get_parameter_dictionary_from_list(
    ...
    def get_parameter_dictionary_from_list(self, parameter_list):
        ...
        excluded_parameter_keys = set(self.fiducial_parameters) - set(
            self.parameters_to_be_updated
        )
            ...
            for (
                ...
                for i in range(self.number_of_bins):
                    ...
                    central_frequency_i = 0.5 * (
                        masked_frequency_array[masked_bin_inds[i]]
                        + masked_frequency_array[masked_bin_inds[i + 1]]
                    )
                    masked_strain_i = masked_strain[
                        masked_bin_inds[i] : masked_bin_inds[i + 1]
                    ]
                    masked_h0_i = masked_h0[masked_bin_inds[i] : masked_bin_inds[i + 1]]
                    masked_psd_i = masked_psd[
                        masked_bin_inds[i] : masked_bin_inds[i + 1]
                    ]
                    masked_frequency_i = masked_frequency_array[
                        masked_bin_inds[i] : masked_bin_inds[i + 1]
                    ]
            ...
            for l_mode, m_mode in self.mode_array:
                ...
                for ell, emm in mode_array_temp:
                    ...
                    for i in range(self.number_of_bins):
                        ...
                        masked_h0_i = masked_h0[
                            masked_bin_inds[i] : masked_bin_inds[i + 1]
                        ]
                        masked_h02_i = masked_h02[
                            masked_bin_inds[i] : masked_bin_inds[i + 1]
                        ]
                        masked_psd_i = masked_psd[
                            masked_bin_inds[i] : masked_bin_inds[i + 1]
                    ...
                    summary_data[interferometer.name][l_mode, m_mode][ell, emm] = dict(
                        b0=b0, b1=b1
                    )
                    if not (l_mode == ell and m_mode == emm):
                        summary_data[interferometer.name][ell, emm][
                            l_mode, m_mode
    ...
    def compute_waveform_ratio(self, parameters):
        ...
        for l_mode, m_mode in self.mode_array:
            ...
            new_polarizations = self.waveform_generator.frequency_domain_strain(
                parameters
            )

            for interferometer in self.interferometers:
                h = interferometer.get_detector_response(
                    new_polarizations, parameters, self.bin_freqs[interferometer.name]
                )
                bi_ifo = self.bin_inds[interferometer.name]
                h0 = self.per_detector_per_mode_fiducial_waveforms[interferometer.name][
                    l_mode, m_mode
                ][bi_ifo]
                waveform_ratio_per_detector = h / h0

                r0 = (
                    waveform_ratio_per_detector[1:] + waveform_ratio_per_detector[:-1]
                ) / 2
                r1 = (
                    waveform_ratio_per_detector[1:] - waveform_ratio_per_detector[:-1]
                ) / (
                    self.bin_freqs[interferometer.name][1:]
                    - self.bin_freqs[interferometer.name][:-1]
                )

                waveform_ratio[interferometer.name][l_mode, m_mode] = [r0, r1]
        parameters["mode_array"] = self.mode_array
        return waveform_ratio

    def calculate_snrs_relative_binning(
        self, waveform_ratio_per_detector, interferometer
    ):
        summary_data_per_interferometer = self.summary_data[interferometer.name]
        d_inner_h = 0
        h_inner_h = 0
        for l_mode, m_mode in self.mode_array:
            a0 = summary_data_per_interferometer[l_mode, m_mode]["a0"]
            a1 = summary_data_per_interferometer[l_mode, m_mode]["a1"]
            ...
            for ell, emm in self.mode_array:
                ...
                h_inner_h_lm += np.sum(
                    b0 * r0 * np.conjugate(r0_ellemm)
                    + b1 * (r0 * np.conjugate(r1_ellemm) + np.conjugate(r0_ellemm) * r1)
                )
        ...
        if self.time_marginalization:
            ...
            duplicated_r0, duplicated_r1, duplicated_fm = np.zeros(
                (3, f.shape[0]), dtype=np.complex
            )
        ...
        mode_array_len = len(self.mode_array)
        return self._CalculatedSNRs(
            d_inner_h=d_inner_h / mode_array_len,
            optimal_snr_squared=optimal_snr_squared / mode_array_len**2,
    ...
    def generate_posterior_sample_from_marginalized_likelihood(self):
        ...
        if any(
            ...
            waveform_ratio = copy.deepcopy(self.compute_waveform_ratio(self.parameters))
        else:
    ...
    def generate_time_sample_from_marginalized_likelihood(self, waveform_ratio=None):
        """
        ...
        for ifo in self.interferometers:
            ...
            duplicated_r0, duplicated_r1, duplicated_fm = np.zeros(
                (3, ifo_length), dtype=np.complex
            )
            for i in range(self.number_of_bins):
                ind = self.bin_inds[ifo.name]
                fm = 0.5 * (
                    self.bin_freqs[ifo.name][1:] + self.bin_freqs[ifo.name][:-1]
                )
                duplicated_fm[ind[i] : ind[i + 1]] = fm[i]
                duplicated_r0[ind[i] : ind[i + 1]] = r0[i]
                duplicated_r1[ind[i] : ind[i + 1]] = r1[i]
        ...
        if self.distance_marginalization:
            time_log_like = self.distance_marginalized_likelihood(d_inner_h, h_inner_h)
        elif self.phase_marginalization:
    ...
    def _rescale_signal(self, waveform_ratio, new_distance):
        for ifo in self.interferometers:
            for l_mode, m_mode in self.mode_array:
                for i in range(2):
                    waveform_ratio[ifo.name][l_mode, m_mode][i] *= (
                        self._ref_dist / new_distance
                    )
    ...
    def generate_phase_sample_from_marginalized_likelihood(self, waveform_ratio=None):
        """
